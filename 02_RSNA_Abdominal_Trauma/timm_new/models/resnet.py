...
from timm_new.layers import DropBlock2d, DropPath, AvgPool2dSame, BlurPool2d, GroupNorm, create_attn, get_attn, \
    get_act_layer, get_norm_layer, create_classifier
from ._builder import build_model_with_cfg
from ._manipulate import checkpoint_seq
from ._registry import register_model, generate_default_cfgs, register_model_deprecations

__all__ = ['ResNet', 'ResNetFewerChannel', 'BasicBlock', 'Bottleneck']  # model_registry will add each entrypoint fn to this
...
class ResNetFewerChannel(nn.Module):
    """ResNet / ResNeXt / SE-ResNeXt / SE-Net
    ...
    def __init__(
        ...
        Args:
            ...
            block_args (dict): Extra kwargs to pass through to block module
        """
        super(ResNetFewerChannel, self).__init__()
        block_args = block_args or dict()
        assert output_stride in (8, 16, 32)
        self.num_classes = num_classes
        self.drop_rate = drop_rate
...
def _create_resnet(variant, pretrained=False, **kwargs):
    if(variant=='resnet10t_fewer'):
        return build_model_with_cfg(ResNetFewerChannel, variant, pretrained, **kwargs)
    else:
...
default_cfgs = generate_default_cfgs({
    ...
    'resnet10t.c3_in1k_fewer': _ttcfg(
        hf_hub_id='timm/',
...
@register_model
def resnet10t_fewer(pretrained=False, **kwargs) -> ResNetFewerChannel:
    """Constructs a ResNet-10-T_fewer model.
    """
    model_args = dict(block=BasicBlock, layers=[1, 1, 1, 1], stem_width=32, stem_type='deep_tiered', avg_down=True)
    return _create_resnet('resnet10t_fewer', pretrained, **dict(model_args, **kwargs))
