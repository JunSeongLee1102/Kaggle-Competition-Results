...
class ResNet(nn.Module):
    ...
    def __init__(self,
        ...
        self.avgpool = nn.AvgPool3d(kernel_size=(5, 5, 5), stride=(3, 3, 3), padding=(1, 1, 1))
    ...
    def forward(self, x):
        ...
        # x11 = self.avgpool(x11) # 추가

        x2 = self.layer2(x1)
        x22 = self.avgpool(x2)     
        x22 = self.avgpool(x22)
        # x22 = self.avgpool(x22) # 추가

        x3 = self.layer3(x2)
        x33 = self.avgpool(x3) 
        x33 = self.avgpool(x33)     
        # x33 = self.avgpool(x33) # 추가

        x4 = self.layer4(x3)
        x44 = self.avgpool(x4)      
        # x44 = self.avgpool(x44)  # 추가
